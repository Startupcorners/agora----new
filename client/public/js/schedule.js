...
window.generate42CalendarDates = generate42CalendarDates;
 ...
 window.generate42CalendarDatesUserTimeZone = generate42CalendarDatesUserTimeZone;
   ...
   function generate42CalendarDatesUserTimeZoneAnnouncements(
     anchorDate,
     isStart,
     offsetSeconds
   ) {
     ...
     for (let i = 0; i < 42; i++) {
       const currentDate = new Date(
         nearestSundayLocal.getTime() + i * oneDayMs
       );
       const adjustedDate = new Date(
         currentDate.getTime() - offsetSeconds * 1000
       ); // Convert back to UTC
     ...
     // Output the dates to the appropriate function based on isStart.
     if (isStart) {
       bubble_fn_listOfStartDatesAnnouncements(dates);
     } else {
       bubble_fn_listOfEndDatesAnnouncements(dates);
     }
   }

   // Make function globally accessible
   window.generate42CalendarDatesUserTimeZoneAnnouncements =
     generate42CalendarDatesUserTimeZoneAnnouncements;
